...
from alphadesign.cfd_analysis import STLWingAnalyzer, WindTunnelRig, F1CFDPipeline

class TestCFDAnalyzer:

    @pytest.fixture
    def mock_stl_file(self):
        # Use a small, physically meaningful three-element wing rather than a
        # unit cube.  The fallback extractor needs enough surface vertices at
        # each element level to estimate chord lengths without a sidecar.
        elements = []
        for element_index in range(3):
            mesh = trimesh.creation.box(
                extents=[0.30, 1.60, 0.03],
                transform=trimesh.transformations.translation_matrix(
                    [0.15, 0.80, element_index * 0.06]
                ),
            )
            for _ in range(2):
