# Surrogate aerodynamic analysis with JSON parameters

> This module is a physics-informed empirical surrogate, not a high-fidelity Navier–Stokes CFD solver. The JSON sidecar supplies millimetre geometry inputs that are normalized to SI units before evaluation. Existing generated reports are legacy artifacts and are not paper evidence without independent validation.
