...
It exists because the phase-one/phase-two results describe a comparison between two search
strategies **under this surrogate**, and that claim is only as good as the surrogate's physical
grounding.

> **Important, corrected 2026-09-17: there are two separate, independent ground-effect
> formulas in this codebase**, and the first fix below was applied to the wrong one.
> `cfd_analysis.py::STLWingAnalyzer.calculate_ground_effect` is used only by
> `evaluate_cfd_perf`, the real-mesh CFD path invoked by `main_pipeline.py`; it has **no
> effect whatsoever** on `artifacts/phase_one/` or `artifacts/phase_two/`. Every result
> generated by `random_search.py`/`ga_only.py` (i.e. everything reported in `todo.md`) goes
> through `evaluate_formula_constratins`, which uses the entirely separate
> `formula_constraints.py::F1FrontWingAnalyzer.compute_ground_effect_physics`. Fixing the
> `cfd_analysis.py` version first and rerunning the pilot produced byte-identical numbers,
> which is what caught the mistake. Both formulas have now been fixed (see
> [Fix: ground-effect formula](#fix-ground-effect-formula) below); the one in
> `formula_constraints.py` is the one that matters for every reported result.
...
Both independent implementations now use the same smooth log-normal-in-h/c curve, fit by least
squares directly to real single-element wing data:
...
**`formula_constraints.py::F1FrontWingAnalyzer.compute_ground_effect_physics`** (the one that
actually matters for every reported result): replaces the piecewise formula (branches at h/c = 0.1
and 0.3) with the fitted curve above, applied to the single `ground_effect_factor` this function
uses for both the main element and flaps (this file has no separate per-element treatment to begin
with, unlike the other one). The `induced_drag_reduction` regime-based approximation in the same
function was left unchanged; it was not part of the literature review and is a separate open item.
Regression tests: `tests/test_ground_effect_formula_constraints.py`.

**`cfd_analysis.py::STLWingAnalyzer.calculate_ground_effect`** (only reachable via
`evaluate_cfd_perf`, the real-mesh CFD path, not any reported result): same fitted curve for the
main element. **What is still a heuristic, not a second data fit**: no isolated per-element
ground-effect dataset exists in the literature reviewed for flap elements (only combined main+flap
system forces are reported). The flap curve reuses the fitted main-element shape and peak location
at reduced amplitude, per the literature's qualitative finding that flaps benefit less from ground
proximity than the main element. This is documented in the function's docstring. Regression tests:
`tests/test_ground_effect_formula.py` (fitted peak location and magnitude, continuity via step size
at two grid resolutions, the floor at 1.0, and that flap attenuation increases with element index).
