...
from .experiment_runner import build_run_metadata, seed_everything
...
class AlphaDesignPipeline:
    def __init__(self, config_path: str = "configs/config.json"):
        ...
        self.run_metadata = build_run_metadata(self._pipeline_experiment_spec())
        with open(os.path.join(self.output_dirs['generation_data'], 'run_metadata.json'), 'w') as handle:
            json.dump(self.run_metadata, handle, indent=2, sort_keys=True)
        ...
        # The policy head is retained for ablation work, but no policy action
        # is applied unless explicitly enabled. Current training data has no
        # action/reward trajectories needed to support an RL claim.
        self.neural_guidance_enabled = bool(
            self.config.get('neural_guidance_enabled', False)
        )
    ...
    def _pipeline_experiment_spec(self):
        from .experiment_runner import ExperimentSpec
        return ExperimentSpec(
            name=("neural_guided_ga_frozen_policy" if self.config.get('neural_guidance_enabled')
                  else "ga_only"),
            seed=self.seed,
            evaluation_budget=(self.config['max_generations'] * self.config['population_size']),
    ...
    def run_single_generation(self, base_params: F1FrontWingParams):
        ...
        new_population = self.generate_next_population_with_progress(
            gen_pbar, fitness_scores
        )
        gen_pbar.close()
    ...
    def generate_next_population_with_progress(
        self, pbar: tqdm, fitness_scores: Optional[List[Dict[str, Any]]] = None
    ) -> List[Dict[str, Any]]:
        new_population = []
        ...
        if fitness_scores is None:
            # Preserve the standalone helper API while allowing the generation
            # loop to reuse its already-computed scores.
    ...
    def apply_neural_guidance_with_progress(self, population: List[Dict[str, Any]], pbar: tqdm):
        if self.neural_network is None or not self.neural_guidance_enabled:
        ...
        try:
            ...
            for individual in population:
                ...
                with torch.no_grad():
                    # Only the value head is trained from the available
                    # generation-level fitness labels. There are no recorded
                    # actions or transition rewards for a policy-gradient loss.
                    policy_output, _ = self.neural_network(param_tensor)
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                for gen_data in recent_generations:
                    ...
                    _, value_output = self.neural_network(param_tensor)
