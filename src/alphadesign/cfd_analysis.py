...
class STLWingAnalyzer:
    ...
    def load_stl_file(self):
        ...
        try:
            ...
            # STL has no unit metadata.  The wing generator exports coordinates
            # in millimetres, while the aerodynamic model is SI-based.  Apply
            # the declared sidecar unit when available and otherwise use a
            # conservative size-based inference for legacy STL files.
            declared_length_unit = (
                self.cfd_params.get('metadata', {}).get('units', {}).get('length')
                if self.cfd_params else None
            )
            self.stl_unit_scale = self.infer_stl_unit_scale(
                self.mesh.bounds, declared_length_unit
            )
            if self.stl_unit_scale != 1.0:
                self.mesh.apply_scale(self.stl_unit_scale)
                print(
                    f"📏 Normalized STL coordinates using scale "
                    f"{self.stl_unit_scale:g} ({declared_length_unit or 'inferred'} units)"
                )
    ...
    @staticmethod
    def infer_stl_unit_scale(bounds, declared_length_unit=None):
        """Return the factor that converts STL coordinates to metres.

        The generator's sidecar declares millimetres.  For old STL-only
        inputs, a wing larger than ten metres is treated as millimetres; a
        physically sized SI mesh is left unchanged.  Ambiguous or unsupported
        units fail closed instead of silently changing the geometry.
        """
        unit_scales = {'m': 1.0, 'meter': 1.0, 'meters': 1.0,
                       'mm': 1e-3, 'millimeter': 1e-3, 'millimeters': 1e-3}
        if declared_length_unit:
            try:
                return unit_scales[declared_length_unit.strip().lower()]
            except KeyError as exc:
                raise ValueError(f"Unsupported STL length unit: {declared_length_unit}") from exc

        extent = np.asarray(bounds)[1] - np.asarray(bounds)[0]
        if not np.all(np.isfinite(extent)) or np.any(extent <= 0):
            raise ValueError("STL has invalid or degenerate bounds")
        return 1e-3 if float(np.max(extent)) > 10.0 else 1.0

    def validate_geometry_sanity(self):
        """Reject dimensions that cannot represent an F1 front wing in SI units."""
        checks = {
            'wingspan_m': (self.wingspan, 0.5, 3.0),
            'reference_area_m2': (self.reference_area, 0.05, 5.0),
            'mean_chord_m': (float(np.mean(self.chord_lengths)), 0.02, 1.0),
        }
        failures = [
            f"{name}={value:.6g} outside [{lower}, {upper}]"
            for name, (value, lower, upper) in checks.items()
            if not np.isfinite(value) or not lower <= value <= upper
        ]
        if failures:
            raise ValueError("Geometry sanity check failed: " + "; ".join(failures))
        return True
    ...
    def multi_element_analysis(self, speed_ms, ground_clearance_mm, wing_angle_deg=0,
        ...
        for i in range(len(self.chord_lengths)):
            ...
            # Forces
            # The aerodynamic coefficient follows the conventional lift sign:
            # positive is upward.  Public downforce is positive when the force
            # acts downward, so retain the signed lift and reverse its sign for
            # the downforce-facing API.
            element_lift_signed = cl_element * dynamic_pressure * element_area
            element_downforce = -element_lift_signed
            ...
            signed_lift += element_lift_signed
            ...
            results['elements'].append({
                ...
                'signed_lift_N': element_lift_signed,
                'drag_N': element_drag,
        ...
        results['signed_lift_N'] = signed_lift
    ...
    def quick_performance_analysis(self, test_speed_kmh=300, ground_clearance=75, wing_angle=0):
        ...
        try:
            ...
            return {
                'total_downforce': None,
                'total_drag': None,
                'efficiency_ratio': None,
                'flow_characteristics': None,
                'stall_margin': None,
                'balance_coefficient': None,
                'valid': False,
