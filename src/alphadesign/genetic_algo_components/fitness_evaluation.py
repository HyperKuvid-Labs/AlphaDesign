...
class FitnessEval:
    ...
    def evaluate_pop_with_progress(self, population, pbar: tqdm):
        ...
        for i, individual in enumerate(population):
            ...
            try:
                ...
                if not constraint_score.get('constraint_valid', False):
                    advanced_metrics['critical_failures'] += 1
                    fitness_scores.append(self._invalid_fitness(
                        constraint_score.get('failure_reason', 'Constraint analysis failed')
                    ))
                    pbar.set_postfix({'Status': 'Constraint Analysis Failed'})
                    pbar.update(1)
                    continue
    ...
    def evaluate_formula_constratins(self, individual):
        try:
            ...
            return {
                ...
                'computed_downforce': None,
                'computed_drag': None,
                'computed_efficiency': None,
                'safety_factor': 0,
    ...
    @staticmethod
    def _invalid_fitness(failure_reason):
        """Represent evaluator failure without synthetic physical values."""
        return {
            'total_fitness': -1000.0,
            'constraint_fitness': 0.0,
            'performance_fitness': 0.0,
            'cfd_fitness': 0.0,
    ...
    def evaluate_cfd_perf(self, individual: Dict, individual_idx: int):
        try:
            ...
            if os.path.exists(expected_path):
                ...
                return self.get_default_cfd_score("Generated STL file is missing")

            if os.path.exists(stl_path):
                params_path = os.path.join(
                    "f1_wing_output", stl_filename.replace('.stl', '_cfd_params.json')
                )
                if not os.path.exists(params_path):
                    return self.get_default_cfd_score(
                        "Generated CFD parameter sidecar is missing"
                    )
                analyzer = STLWingAnalyzer(stl_path, cfd_params_json=params_path)
            ...
            return self.get_default_cfd_score(str(e))
    ...
    def get_default_cfd_score(self, failure_reason="CFD analysis failed"):
        """Return an explicit invalid result; never fabricate aerodynamic values."""
        return {
            'cfd_downforce': None,
            'cfd_drag': None,
            'cfd_efficiency': None,
    ...
    def combine_scores(self, constraint_score, cfd_score):
        ...
        # Do not let a failed CFD call look like a partially successful design.
        if cfd_score.get('failure_reason'):
            total_fitness = -1000.0
        ...
        return {
            ...
            'cfd_downforce': cfd_score.get('cfd_downforce'),
            'cfd_efficiency': cfd_score.get('cfd_efficiency'),
            'stall_margin': cfd_score.get('stall_margin'),
            ...
            'failure_reason': cfd_score.get('failure_reason'),
